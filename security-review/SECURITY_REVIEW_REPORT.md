...
| **Safe to deploy?** | **Yes** to staging/prod from this branch *after* a normal review — all changes are behaviour-preserving except two intentional fail-closed hardenings (compose now requires `POSTGRES_PASSWORD`; container now runs as non-root). No DB migration. See `DEPLOYMENT_AND_ROLLBACK.md`. |
...
| **M2** | Med | High | Weak credential (CWE-1392/258) | `docker-compose.yml:18,42` (pre-fix) | `${POSTGRES_PASSWORD:-squishybot_dev}` weak fallback | DB on shared net comes up with a guessable password if the var is unset | `${POSTGRES_PASSWORD:?…}` — fail loudly | ✅ Fixed (`a185988`) |
