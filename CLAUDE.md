...
| `activity_message_stats` | Hour-bucketed per-user/per-channel message counts (messages, words, chars, attachments, mentions, replies). No message content stored. Feature-flagged (`feature.activity_stats`, default OFF). `channel_kind` (`auto_voice`/`auto_text`/NULL) marks ephemeral auto-channel rows so the panel can group them instead of listing dead channel IDs. |
...
| `activity_voice_sessions` | One row per voice stay (join → leave); `rolled_up_to` watermarks how much of an open session has already been folded into `activity_voice_stats`, so a bot restart can't double-count. Carries `channel_kind`; `channel_name` refreshes from the live channel each rollup tick so renames stick. |
| `activity_voice_stats` | Hour-bucketed voice seconds — the source for voice heatmaps/leaderboards. Carries `channel_kind` (see `activity_message_stats`). |
...
| `src/services/activity/channelKinds.ts` | Auto-channel classification for stats — `classifyChannelKind` (sync record-time lookup vs the settings.ts auto-channel registry; static VCs excluded), `stampActivityChannelKinds` (teardown re-stamp on every channel-pair delete path), `sweepLegacyChannelKinds` (startup/flag-on sweep of NULL-kind rows for channels that no longer exist). |
