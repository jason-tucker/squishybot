...
### Added
- **`staff.grant` + `staff.revoke` RPC verbs (Wave 7b).** Two new handlers under `src/services/rpc/handlers/staff/` register against the bot-side command-bus subscriber so the panel can grant or revoke any of the 7 staff roles over the HMAC-signed `cmd.squishy.*` envelope. Params: `{ userId, roleKey }` — `roleKey` accepts either the full `staff.role.*` `bot_settings` key or the bare slug (`tier_1`, `help_desk`, …) and is normalized via the existing `staffRoles.ts` registry. Both verbs look the role ID up in `bot_settings`, fetch the guild + member, and call `member.roles.add` / `member.roles.remove` with an explicit audit reason. Reply taxonomy is stable + machine-readable so the panel can render targeted errors: `bad-params`, `role-key-unknown`, `role-not-linked`, `guild-unavailable`, `role-missing-in-guild`, `user-not-found`, `discord-error`. Grants are idempotent — `alreadyHad: true` in the reply when the member already had the role — and so are revokes (`didNotHave: true`). New `handlers/staff/index.ts` is a side-effect barrel that pulls both files in via one boot-path import from `ready.ts`, matching the existing `echo` handler pattern.
