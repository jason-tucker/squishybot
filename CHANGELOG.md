...
### Added
- **`staff.grant` + `staff.revoke` RPC verbs (Wave 7b).** Two new handlers under `src/services/rpc/handlers/staff/`. Params `{userId, roleKey}` (full `staff.role.*` key or bare slug). Idempotent (`alreadyHad`/`didNotHave` flags). Error taxonomy: `bad-params`, `role-key-unknown`, `role-not-linked`, `guild-unavailable`, `role-missing-in-guild`, `user-not-found`, `discord-error`.
- **Admin RPC verbs — `admin.reload_caches`, `admin.orphan_scan`, `admin.reconciler_run` (Wave 7b).** Mirror the `/sudo → Settings → Debug` Discord buttons. `reload_caches` runs settings+games+social-feeds+bot_owner in parallel. `orphan_scan` walks 5 tables read-only. `reconciler_run` thin pass-through to `runReconciler(client)`. All wired in `ready.ts` via a side-effect barrel import.
