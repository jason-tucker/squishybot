...
### Migration (one-time, run 2026-05-19)
- **Game ping-role cleanup script — ran against live Discord + DB.** Two scripts under `scripts/`: (1) `migrate-game-roles.ts` strips the " LFG" suffix from any `games.ping_role_id` whose Discord name ends in " LFG", and converts the "view role" concept (`games.role_id` column) to per-channel permission overwrites before deleting the role. (2) `rename-game-ping-roles.ts` is a per-row allowlist for the case/formatting/abbreviation mismatches the first script doesn't touch (e.g. `deadlock` → `Deadlock`, `WoW` → `World of Warcraft`, `PoE++` → `Path of Exile 2`). Both are REST-only (no gateway connection, no conflict with the running bot), dry-run by default, `--apply` required to commit, rate-limited at 250ms/write. Result on live data: 1 LFG-suffix rename ("Slay The Spire 2 LFG" → "Slay The Spire 2"), 1 view role deleted (had 0 holders), 9 mismatch renames. Closes botpanel #211 and the bot side of #194 (view-role concept).
