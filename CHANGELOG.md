...
### Added
- **`admin.orphan_cleanup` RPC verb** — backs the new panel `/squishy/cleanup` page. Mirrors the in-Discord `sudo:set:debug:cleanup_orphans` button: walks `auto_channels`, `hub_channels`, `auto_thread_channels`, `archived_channels` and deletes rows whose Discord references are **entirely** missing. Rows with partial orphans are left alone (the user repairs those via the Games panel). Returns `{deleted, byTable: {auto_channels: n, hub_channels: n, ...}}` so the panel can show a per-table summary. Reloads the settings cache after deletes to keep in-memory state consistent.
