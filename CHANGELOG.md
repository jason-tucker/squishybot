...
- **Voice control panel — Hosts / Hide-Show / Randomize now defer before their REST-heavy work.** `voiceControl.ts`'s `hosts` branch (previously up to 24 *sequential* `guild.members.fetch()` calls before any ack) now defers first and fetches hosts in parallel via `Promise.all`; the `hide`/`show` branch (sequential `permissionOverwrites.edit()` calls) and `randomize` branch (channel fetch + `setName()` calls) now defer before doing that work. `requireControl`/`requireOwnerOrSudo` were made defer-aware (reply when not yet acknowledged, followUp otherwise) so both deferred and non-deferred call sites keep working.
- **`/sudo → Settings` selects and modals now defer before the sudo check and before slow work.** `handleSettingsChannelSelect`, `handleSettingsRoleSelect`, `handleSettingsUserSelect`, `handleSettingsStringSelect`, and `handleSettingsModalSubmit` in `sudoSettings.ts` all now defer first, mirroring `handleSettingsButton`'s established pattern; `showModal()` branches (per-hub defaults, per-hub lockdown duration, auto-thread template) are routed before the defer with their own sudo check, since `showModal()` can't follow a defer.
- **Adding a social feed (`sudo:set:social:add_submit`) no longer risks timing out during the 15s feed fetch.** The modal submit handler defers immediately (covered by the new top-of-function defer in `handleSettingsModalSubmit`), so the `fetchAndParse(url)` call that seeds the dedupe key can safely take its full timeout.
- **Report review buttons (`report_approve_*` / `report_reject_*`) now defer before the bot-owner check.** `reportReview.ts`'s `handleReportReview` defers first since `isBotOwner()` can do a live Discord API fetch on a cold 60s cache; the denial now goes via `followUp`.
...
### Performance
- **`voiceStateUpdate` skips Postgres for unmanaged channels.** Both the join and leave branches queried `auto_channels` on every voice join/leave/move guild-wide; they now short-circuit via the in-memory auto-channel voice cache (same pattern the `presenceUpdate` hot path already used). The hub-join race guard is preserved — `handleHubJoin` re-checks `auto_channels` itself.
- **Reaction-role handler checks its in-memory config before fetching partials.** Every reaction added/removed on any uncached (pre-restart) message previously paid a REST message-fetch just to discover it wasn't a reaction-role message; the cache + emoji-mapping check now runs first, so partial fetches only happen for actual reaction-role messages.
- **Hub joins no longer run a full-table `auto_channels` query for name de-duplication.** `handleHubJoin` reads the tracked voice-channel ID set from the in-memory cache (new `listAutoChannelVoiceIds()` in `services/settings.ts`).
- **Startup reconciler passes the already-selected record to `maybeRenameChannel`** instead of the channel ID, saving one `auto_channels` query per room on every boot/reconcile.

### Fixed
- **In-memory cache reloads are now atomic (query-then-swap).** `loadSettings()`, `loadGames()`, `loadReactionRoles()`, and `loadSocialFeeds()` previously cleared their caches *before* awaiting the DB queries, so every reload (boot, RPC `admin.reload_caches`, panel cache-invalidate) opened a window where settings/sudo/hub/auto-channel/games/reaction-role state was empty — and a transient DB error during a reload silently wiped the affected cache until the next successful reload (each query swallowed failures via `.catch(() => [])`). All four loaders now query first and swap the live caches in one synchronous step; on failure the previous state stays live and the error propagates to the caller's existing catch/log.
- **`presence.last_used_at` no longer floods the `setting_changes` audit table and the `setting_changed` event channel.** The presence service rewrites this internal bookkeeping key every ~5 minutes while the bot is in use, which inserted an audit row and published a Redis event each time — unbounded noise growth with zero operator value. `setSetting()` gained an `opts.audit` flag (default `true`, i.e. behavior unchanged for every operator-facing setting) and the presence write opts out.
- **Deploy: the "Deploy to VPS" step no longer red-X's on a watchtower race.** The `squishybot` service is labelled `com.centurylinklabs.watchtower.enable=true`, so watchtower independently recreates the container when a new `:latest` lands on GHCR — at the same moment the CI SSH step runs `docker compose up -d`. The two collided intermittently with `Error response from daemon: removal of container … is already in progress`, failing the deploy job even though watchtower had already brought the bot up on the new image. The `compose up` is now wrapped in a 5-attempt retry loop (8s backoff); once watchtower's recreate settles, `up -d` converges to a no-op on the running `:latest` and succeeds. Both deployers stay enabled — they just stop fighting.
